...
def main(args):
    ...
    data_root = '../../../datasets/DLPFC/' + sample_name
    ...
    y = np.loadtxt(data_root + '/' + sample_name + "_truth.csv", delimiter=",")
    ...
    try:
        clustering(adata, n_clusters, radius=radius, used_obsm=used_obsm, method='louvain', start=0.1, end=2.0,
                   increment=0.01)
    except Exception as e:
    ...
    adata.obs[cols].to_csv(cluster_path + 'SEDR_na.tsv', sep='\t', index=True)
