...
def main(args):
    ...
    result_path = '../output/HER2ST/' + section_id + '/'
    cluster_path = '../cluster/HER2ST/' + section_id + '/'
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    if not os.path.exists(cluster_path):
        os.makedirs(cluster_path)

    input_dir = '../../../datasets/HER2ST/' + section_id
    ...
    try:
        clustering(adata, n_clusters, method='leiden', start=0.1, end=2.0, increment=0.01)
    except Exception as e:
        adata.obs['leiden'] = None

    try:
        clustering(adata, n_clusters, method='louvain', start=0.1, end=3.0, increment=0.01)
    except Exception as e:
        adata.obs['louvain'] = None

    clustering(adata, n_clusters, method='kmeans')
