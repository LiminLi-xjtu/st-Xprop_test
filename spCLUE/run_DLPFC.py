...
def main(args):
    ...
    ##################################################################################
    result_path = '../output/DLPFC/' + sample_name + '/'
    ...
    file_fold = '../../../datasets/DLPFC/' + str(sample_name)  # please replace 'file_fold' with the download path
    adata = sc.read_visium(file_fold, count_file=str(sample_name) + '_filtered_feature_bc_matrix.h5', load_images=True)

    adata.var_names_make_unique()
    print(adata)

    y = np.loadtxt(file_fold + '/' + sample_name + "_truth.csv", delimiter=",")
    ...
    spCLUE.clustering(adata, n_clusters, used_obsm=used_obsm, method='mclust', start=0.1, end=3.0, increment=0.01, refinement=True)
    try:
        spCLUE.clustering(adata, n_clusters, used_obsm=used_obsm, method='leiden', start=0.1, end=2.0, increment=0.01, refinement=False)
    except Exception as e:
        adata.obs['leiden'] = None

    try:
        spCLUE.clustering(adata, n_clusters, used_obsm=used_obsm, method='louvain', start=0.1, end=2.0, increment=0.01, refinement=False)
    except Exception as e:
        adata.obs['louvain'] = None

    spCLUE.clustering(adata, n_clusters, used_obsm=used_obsm, method='kmeans', refinement=False)
    ...
    adata.obs[cols].to_csv(cluster_path + 'spCLUE_na.tsv', sep='\t', index=True)
...
if __name__ == '__main__':
   ...
   parser.add_argument('--slice', type=str, default='151671', help='name of the dataset')
