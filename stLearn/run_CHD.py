...
def main(args):
    ...
    BASE_PATH = Path('../../datasets/CHD/'+sample)
    file_fold = '../../datasets/CHD/' + sample
    ...
    ##################################################################################
    result_path = './output/CHD/' + sample + '/'
    cluster_path = './cluster/CHD/' + sample + '/'
    ...
    # data.obs['ground_truth'] = df.iloc[:, 1].values

    data = sc.read_h5ad(f'{file_fold}/{sample}_reset.h5ad')
    data.obs['ground_truth'] = data.obs['region']
    n_clusters = len(np.unique(data.obs['ground_truth']))
     

    from get_uns import prepare_uns_spatial,SME_normalize_for_stereo

    data = prepare_uns_spatial(data, f'{file_fold}/chicken_heart_spatial_RNAseq_{sample}_image.tif')
    ...
    clustering(data_, n_clusters, radius=radius, method='mclust',
               refinement=True)  # For DLPFC dataset, we use optional refinement step.
    
    try:
        clustering(data_, n_clusters, radius=radius, method='leiden', start=0.1, end=2.0, increment=0.01, refinement=False)
    except Exception as e:
        data_.obs['leiden'] = None
    
    try:
        clustering(data_, n_clusters, radius=radius, method='louvain', start=0.1, end=3.0, increment=0.01, refinement=False)
    except Exception as e:
        data_.obs['louvain'] = None
    
    clustering(data_, n_clusters, radius=radius, method='kmeans', refinement=False)
