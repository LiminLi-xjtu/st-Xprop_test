...
echo "slice,ari_kmeans" > "$output_file"
...
# num_clusters=(6)
output_file="../evaluation/CHD/SpaMask.csv"
echo "slice,ari_kmeans" > "$output_file"
...
for i in "${!slices[@]}"; do
    ...
    # n_clusters=${num_clusters[$i]}
  echo "Running with slice=$slice"
  python run_CHD.py --slice "$slice" 2>/dev/null | tail -n 1 >> "$output_file"
done
...
output_file="../evaluation/HER2ST/SpaMask.csv"
echo "slice,ari_kmeans" > "$output_file"
