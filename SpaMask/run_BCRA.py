...
def load_adata(section_ids, k_cutoff, rad_cutoff, model, n_top_genes):
    ...
    for section_id in section_ids:
        ...
        adata = sc.read_visium(path, count_file=section_id+"_filtered_feature_bc_matrix.h5")
        ...
        df_label = pd.read_csv(f"{path}/{section_id}_truth.txt", sep='\t', header=None, index_col=0)
        df_label.columns = ['layer_guess']
        adata.obs['Ground Truth'] = df_label['layer_guess']
...
if __name__ == '__main__':
    ...
    eva_path = './evaluation/BRCA/' + args.slice + '/'
