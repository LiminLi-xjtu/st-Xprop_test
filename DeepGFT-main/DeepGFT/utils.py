...
def denoising(X, adj, att):
    ...
    Y = np.array(X + 0.2 / lambda_max * np.matmul(csr_adj.toarray(), X))
...
def f2s_gene(adata, gene_signal=1024, c1=0.05, fast_mode=True):
    ...
    gene_freq_mtx, gene_eigvecs_T, gene_eigvals = GraphFourierTransform(spot_net, adata.X.copy().todense(),
                                                                        n_GFT=gene_signal, fast_mode=fast_mode)
    gene_freq_mtx = np.matmul(np.diag(1 / (1 + c1 * gene_eigvals)), gene_freq_mtx)
    adata.uns['gene_freq'] = gene_freq_mtx
    return gene_freq_mtx.T, gene_eigvecs_T, gene_eigvals


def f2s_spot(adata, spot_signal=1024, c2=0.05, middle=10, fast_mode=True):
    ...
    spot_freq_mtx, spot_eigvecs_T, spot_eigvals = GraphFourierTransform(gene_net, adata.X.todense().T,
                                                                        ...
                                                                        middle=middle, fast_mode=fast_mode)
    spot_freq_mtx = np.matmul(np.diag(1 / (1 + c2 * spot_eigvals)), spot_freq_mtx)
    adata.uns['signal'] = pd.DataFrame(spot_freq_mtx.T, index=adata.obs.index)
    return spot_freq_mtx.T, spot_eigvecs_T, spot_eigvals


def GraphFourierTransform(net, X, n_GFT=256, middle_pass=False, middle=5, fast_mode=True):
    ...
    if fast_mode:
        eigvals, eigvecs = ss.linalg.eigsh(lap_mtx.astype('double').toarray(),
                                           k=lap_mtx.shape[0],
                                           which='SM',
                                           v0=[1 / np.sqrt(net.shape[0])] * net.shape[0])
        eigvals, eigvecs = eigvals[:n_GFT], eigvecs[:, :n_GFT]
    else:
...
def obtain_pre_spotnet(adata, adata_raw, pre_feature=False, k_cutoff=6, res_pre=0.6):
    ...
    # sc.pp.neighbors(raw)
    sc.pp.pca(raw, svd_solver='randomized')
    sc.pp.neighbors(raw,method='gauss')
...
def svg(adata, svg_method='gft_top', n_top=3000, csvg=0.0001, ratio_low_freq=15, smoothing=True):
    """
...
def clustering(adata, n_clusters=7, radius=50, used_obsm='emb', method='mclust', start=0.1, end=3.0, increment=0.01, res_test=None,
               refinement=False):
    """\
