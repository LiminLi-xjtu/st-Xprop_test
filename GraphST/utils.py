...
def clustering(adata, n_clusters=7, radius=50, key='emb', method='mclust', start=0.1, end=3.0, increment=0.01, refinement=False, res_test=None, modelNames='EEE'):
    """\
    ...
    embedding = pca.fit_transform(np.array(adata.obsm['emb'].copy(), dtype=float))
    adata.obsm['emb_pca2'] = embedding
    
    adata.obsm['emb_pca'] = adata.obsm['emb']
    
    if method == 'mclust':
        adata = mclust_R(adata, used_obsm='emb_pca2', num_cluster=n_clusters,modelNames=modelNames)
        adata.obs['domain'] = adata.obs['mclust']
    elif method == 'kmeans':
        kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(adata.obsm['emb_pca'])
