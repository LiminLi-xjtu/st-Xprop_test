...
output_file="../evaluation/DLPFC/SpaGCN.csv"
echo "slice,ari_louvain_na,ref_ari_louvain_na,ari_louvain,ref_ari_louvain" > "$output_file"
...
for i in "${!slices[@]}"; do
    ...
    # n_clusters=${num_clusters[$i]}
  echo "Running with slice=$slice"
  python ./SpaGCN/run_CHD.py --slice "$slice"
done

##########################################################################################################
slices=("A1" "B1" "C1" "D1" "E1" "F1" "G2" "H1")

output_file="../evaluation/her2st/SpaGCN.csv"
echo "slice,ari_louvain,ref_ari_louvain" > "$output_file"
