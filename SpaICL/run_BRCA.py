...
def main(args):
    ...
    with open('./SpaICL-main/config/BRCA_CLFS.yaml', 'r', encoding='utf-8') as f:
        config = yaml.load(f.read(), Loader=yaml.FullLoader)


    path = f'../../datasets/BRCA'
    
    proj_name = args.slice
    n_clusters = args.n_clusters    
    result_path = './output/' + proj_name + '/'
    cluster_path = './cluster/' + proj_name + '/'
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    if not os.path.exists(cluster_path):
        os.makedirs(cluster_path)

    adata = sc.read_visium(path, count_file=proj_name+"_filtered_feature_bc_matrix.h5")
    adata.var_names_make_unique()

    ##### Load layer_guess label, if have
    df_label = pd.read_csv(f"{path}/{proj_name}_truth.txt", sep='\t', header=None, index_col=0)
    df_label.columns = ['layer_guess']
    adata.obs['layer_guess'] = df_label['layer_guess']
    n_clusters = len(np.unique(adata.obs['layer_guess']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--slice', type=str, default='BRCA', help='name of the dataset')
    ...
    df.to_csv(eva_path + "/SpaICL.csv", index=False)
