...
def main(args):
    ...
    path = '../../datasets/CHD/' + proj_name
    # adata = sc.read_visium(path, count_file=name+'_filtered_feature_bc_matrix.h5')


    result_path = './output/CHD/' + proj_name + '/'
    cluster_path = './cluster/CHD/' + proj_name + '/'
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    if not os.path.exists(cluster_path):
        os.makedirs(cluster_path)
        
    adata = sc.read_h5ad(f'{path}/{proj_name}_reset.h5ad')
    adata.obs_names = adata.obs_names + '-1'
    adata.var_names_make_unique()
    

    ##### Load layer_guess label, if have
    adata.obs['layer_guess'] = adata.obs['region']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--slice', type=str, default='D14', help='name of the dataset') #'D14_estimated'
