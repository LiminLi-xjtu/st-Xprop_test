...
def main(args):
    ...
    result_path = '../output/DLPFC/' + proj_name + '/'
    cluster_path = '../cluster/DLPFC/' + proj_name + '/'
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    if not os.path.exists(cluster_path):
        os.makedirs(cluster_path)
        
    path = f'../../../datasets/DLPFC/{proj_name}' 
    adata = sc.read_visium(path, count_file=f'{proj_name}_filtered_feature_bc_matrix.h5')
